        ...
        if do_qrg:
            ...
            for idx in elite_idx:
                ...
                for d in qrg_optimize_dims:
                    # 按维度设置不同的旋转强度
                    if d == 0:
                        theta_use = theta_t * 0.8   # lr 更稳一点
                    elif d == 1:
                        theta_use = theta_t * 1.0   # dice_weight 正常
                    else:
                        theta_use = theta_t

                    # 二值/方向化启发：看当前值在最优值左侧还是右侧
                    direction = 1.0 if best_position[d] >= Xi[d] else -1.0

                    # 量子旋转门式微调，本质是按 theta 决定步长
                    step_scale = math.sin(theta_use) * (ub[d] - lb[d]) * 0.15
                    perturb = direction * step_scale * (0.5 + rng.random())

                    Xnew[d] = Xi[d] + perturb

                Xnew = _ensure_bounds(Xnew, lb, ub)
                qrg_pop[idx] = Xnew
