...
OUT_ROOT = Path("outputs/final_compare_7methods")
...
METHODS = [
    "adaptive_qrghgs",   # 主方法：mid_v3_perturb
    "qrghgs_baseline",   # 原始版：before_no_perturb
    "random",
    "tpe",
    "pso",
    "ga",
...
def real_evaluate(method, idx, row):
    run_name = f"{method}_trial{idx:03d}"
...
def save_global_summary(all_real_path: Path, summary_path: Path):
    if not all_real_path.exists():
        return
    df = pd.read_csv(all_real_path)
    if len(df) == 0:
        return
    summary = df.groupby("method")[["best_val_dice", "test_dice", "time_sec"]].agg(["mean", "max", "std"])
...
def run_random_surrogate(obj, budget, seed):
    ...
    for i in range(budget):
        ...
        p = vec_to_params(x)
        rows.append({"method": "random", "trial_idx": i, **p, "pred_mean": mean, "pred_std": std, "acq": acq})
...
def run_tpe_surrogate(obj, budget, seed):
    ...
    def objective(trial):
        ...
        x = params_to_vec({"lr": lr, "dice_weight": dice_weight, "batch_size": batch_size})
        acq, mean, std = obj.eval_vec(x)
        p = vec_to_params(x)
        rows.append({"method": "tpe", "trial_idx": trial.number, **p, "pred_mean": mean, "pred_std": std, "acq": acq})
        return acq
...
def run_pso_surrogate(obj, budget, seed, pop_size=6):
    ...
    for _ in range(max_iters):
        for i in range(pop_size):
            ...
            p = vec_to_params(X[i])
            rows.append({"method": "pso", "trial_idx": trial_idx, **p, "pred_mean": mean, "pred_std": std, "acq": acq})
...
def run_ga_surrogate(obj, budget, seed, pop_size=6):
    ...
    for _ in range(max_gens):
        ...
        for i in range(pop_size):
            ...
            rows.append({"method": "ga", "trial_idx": trial_idx, **p, "pred_mean": mean, "pred_std": std, "acq": acq})
            fits.append(acq)
            trial_idx += 1
            if trial_idx >= budget:
                return pd.DataFrame(rows)
...
def run_hgs_surrogate(obj, budget, seed, pop_size=6):
    ...
    for i in range(pop_size):
        acq, mean, std = obj.eval_vec(pop[i])
        p = vec_to_params(pop[i])
        rows.append({"method": "hgs", "trial_idx": trial_idx, **p, "pred_mean": mean, "pred_std": std, "acq": acq})
        fit[i] = acq
        trial_idx += 1
        if trial_idx >= budget:
            return pd.DataFrame(rows)
...
# =========================================================
# 4. 基线 QRG-HGS 与主方法 Adaptive QRG-HGS
# =========================================================
def run_qrghgs_variant(obj, budget, seed, method_name, cfg):
    rng = np.random.default_rng(seed)
    max_iters = max(1, math.ceil(budget / POP_SIZE))
    ...
    for i in range(POP_SIZE):
        acq, mean, std = obj.eval_vec(pop[i])
        p = vec_to_params(pop[i])
        rows.append({"method": method_name, "trial_idx": trial_idx, **p, "pred_mean": mean, "pred_std": std, "acq": acq})
        fit[i] = acq
        trial_idx += 1
        if trial_idx >= budget:
            return pd.DataFrame(rows)
    ...
    for it in range(max_iters):
        ...
        if cfg["use_perturb"]:
            if cfg["noise_mode"] == "fixed":
                base_noise_ratio = cfg["base_noise_fixed"]
                qrg_noise_ratio = cfg["qrg_noise_fixed"]
                restart_noise_ratio = cfg["restart_noise_fixed"]
            else:
                gamma = cfg["noise_gamma"]
                alpha = progress ** gamma
                base_noise_ratio = cfg["base_noise_min"] + (cfg["base_noise_max"] - cfg["base_noise_min"]) * alpha
                qrg_noise_ratio = cfg["qrg_noise_min"] + (cfg["qrg_noise_max"] - cfg["qrg_noise_min"]) * alpha
                restart_noise_ratio = cfg["restart_noise_min"] + (cfg["restart_noise_max"] - cfg["restart_noise_min"]) * alpha
        else:
            base_noise_ratio = qrg_noise_ratio = restart_noise_ratio = 0.0
...
def main():
    ...
    all_search = pd.read_csv(all_search_path) if all_search_path.exists() else pd.DataFrame()
    all_real = pd.read_csv(all_real_path) if all_real_path.exists() else pd.DataFrame()

    for method in METHODS:
        print(f"\n========== RUN {method.upper()} ==========")

        search_csv = OUT_ROOT / f"{method}_surrogate_search.csv"
        topk_csv = OUT_ROOT / f"{method}_topk_candidates.csv"
        real_csv = OUT_ROOT / f"{method}_real_eval.csv"

        # 1) 代理搜索
        if search_csv.exists():
            df = pd.read_csv(search_csv)
            print(f"[SKIP SEARCH] {search_csv.name}")
        else:
            if method == "random":
            ...
            elif method in ["adaptive_qrghgs", "qrghgs_baseline"]:
                cfg = get_method_config(method)
                df = run_qrghgs_variant(obj, SURROGATE_BUDGET, SEED, method, cfg)
            else:
                continue
        ...
        if topk_csv.exists():
            ...
            print(f"[SKIP TOPK] {topk_csv.name}")
        else:
        ...
        for i, (_, row) in enumerate(topk.iterrows()):
            run_name = f"{method}_trial{i:03d}"
            if run_name in done_run_names:
                print(f"[SKIP REAL ROW] {run_name}")
                continue
            new_rows.append(real_evaluate(method, i, row))
        ...
        df["method"] = method
        if len(all_search) > 0:
            all_search = all_search[all_search["method"] != method]
